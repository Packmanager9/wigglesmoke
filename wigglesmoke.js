...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Smoke{
        constructor(){
            this.center = new Circle(TIP_engine.x, TIP_engine.y, 1, "transparent")
            this.smoketimer = 1
            this.click = 0
            this.balls = []
            this.r = 125
        ...
        makedot(){
            if(this.click%this.smoketimer == 0){
                this.r+=.2
                this.r%=255
                this.g+=.3
                this.g%=255
                this.b+=.5
                this.b%=255
                let dot = new Circle(this.center.x, this.center.y, 7, `rgba(${this.r},${this.g},${this.b}, .5)`, (Math.random()-.5)*9,(Math.random()-.5)*9, .99)
                dot.r = this.r
                dot.g = this.g
                dot.b = this.b
                dot.decay = .99-(Math.random()*.02)
                dot.links = []
                for(let t = 0;t<this.balls.length;t++){
                    let line = new LineOP(this.balls[t], dot, `rgba(${(this.r+this.balls[t].r)*.5},${(this.g+this.balls[t].g)*.5},${(this.b+this.balls[t].b)*.5}, .1)`, 4)
                    if(line.sqrDis() < 1000){
                        dot.links.push(line)
                    }
                }
        ...
        draw(){
            this.center.x = TIP_engine.x
            this.center.y = TIP_engine.y
            this.click++
            this.makedot()
            this.makedot()
            this.makedot()
            ...
            for(let t = 0;t<this.balls.length;t++){
                ...
                this.balls[t].draw()
                this.balls[t].radius*=this.balls[t].decay
            }
            for(let t = 0;t<this.balls.length;t++){
                for(let k = 0;k<this.balls[t].links.length;k++){
                    this.balls[t].links[k].draw()
                    this.balls[t].links[k].width *= .985
                }
            for(let t = 0;t<this.balls.length;t++){
                if(this.balls[t].radius<5){
